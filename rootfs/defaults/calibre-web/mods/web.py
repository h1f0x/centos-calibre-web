...
@app.route("/lookup/<search_title>")
@login_required_if_no_ano
def lookup_book(search_title):
    conn = HTTPConnection('www.orellfuessli.ch:443')
    search_request = '/suche?sq=' + quote(search_title) + '&filterPATHROOT=2&asn=true&asnlinkname=Kategorien_B%C3%BCcher'

    conn.request('GET', search_request)
    resp = conn.get_response()

    soup = BeautifulSoup(resp.read(), features="lxml")
    product_list = soup.find('div', {'class':'productList socs_product_list'})
    items = product_list.find_all('div', {'class': 'row product'})

    books = []

    for item in items:
        book = {}
        book['title'] = item.find('img')['title'].split(' / ')[0]
        book['image'] = item.find('img')['src'].replace('-08.jpg', '-00.jpg')
        try:
            book['authors'] = item.find('div', {'class': 'ncAuthor'}).text.split('\n')[3].split(', ')
        except:
            book['authors'] = ''

        try:
            release = item.find('div', {'class': 'show-for-medium'}).text.split('\n')[2].replace(' ', '').replace('Erschienen', '').split('.')
            book['release'] = release[2] + '-' + release[1] + '-' + release[0]
        except:
            book['release'] = ''

        try:
            book['series'] = item.find('div', {'class': 'show-for-small-only'}).text.split('\n')[1].split('/')[1].replace('"','').lstrip()
            book['volume'] = int(
                item.find('div', {'class': 'show-for-small-only'}).text.split('\n')[1].split('/')[0].replace('Band ',
                                                                                                             ''))
        except:
            book['series'] = ''
            book['volume'] = ''

        book['link'] = item.find('a', {'class': 'lads'})['href']

        conn.request('GET', book['link'])
        detail_resp = conn.get_response()

        soup_detail = BeautifulSoup(detail_resp.read(), features="lxml")
        detail = soup_detail.find('div', {'class': 'row storeWindow'})
        try:
            book['description'] = detail.find('dd', {'class': 'cTypeBeschreibung'}).text.replace('\n\n\n','').replace('\n  ', '').lstrip().replace('„','').replace('\u2026', '').replace('\u00a0', '').replace('\u201c','')
        except:
            book['description'] = ''

        try:
            book['publisher'] = detail.find('tr', {'data-test': 'produktdetails_verlag'}).text.replace('\n','').replace('Verlag','').strip()
        except:
            book['publisher'] = ''

        rating = detail.find('div', {'data-test': 'bewertungArtikel'})
        book['rating'] = 5 - (len(str(rating).split('inactive')) -1)

        tags = soup_detail.find('nav', {'component': 'artikeldetails-bereich'})
        tags = tags.find('ul', {'class': 'breadcrumbs'}).text.replace('\nB\u00fccher\n','').rstrip().split('\n')
        for tag in tags:
            if 'Nach' in tag:
                tags.remove(tag)
